...
module "policy" {
  ...
  name   = lower("atlantis-${data.aws_eks_cluster.eks.name}")
  path   = "/"
...
module "role" {
  ...
  role_name                     = lower("atlantis-${data.aws_eks_cluster.eks.name}")
  provider_url                  = local.oidc_provider_url
  oidc_fully_qualified_subjects = [lower("system:serviceaccount:atlantis:atlantis")]
